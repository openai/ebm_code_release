...
def get_weight(name, shape, gain=np.sqrt(2), use_wscale=False, fan_in=None, spec_norm=False, zero=False, fc=False):
    ...
    if use_wscale:
        ...
        if zero:
            var = tf.get_variable(shape=shape, name=name + 'weight', initializer=tf.initializers.random_normal(stddev=1e-10))
        ...
        if zero:
            ...
            var = tf.get_variable(name + 'weight', shape=shape, initializer=tf.contrib.layers.xavier_initializer(dtype=tf.float32))
...
def optimistic_restore(session, save_file, v_prefix=None):
    ...
    var_names = sorted([(var.name, var.name.split(':')[0]) for var in tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES)
...
def optimistic_remap_restore(session, save_file, v_prefix):
    ...
    vars_list = tf.get_collection(tf.GraphKeys.GLOBAL_VARIABLES, scope='context_{}'.format(v_prefix))
    var_names = sorted([(var.name.split(':')[0], var) for var in vars_list if ((var.name.split(':')[0]).replace('context_{}'.format(v_prefix), 'context_0') in saved_shapes)])
    restore_vars = []
    ...
    with tf.variable_scope('', reuse=True):
        for saved_var_name, curr_var in var_names:
            var_shape = curr_var.get_shape().as_list()
            saved_var_name = saved_var_name.replace('context_{}'.format(v_prefix), 'context_0')
            if var_shape == saved_shapes[saved_var_name]:
...
def init_conv_weight(weights, scope, k, c_in, c_out, spec_norm=True, zero=False, scale=1.0, classes=1):
    ...
    with tf.variable_scope(scope):
        ...
        conv_weights['cb'] = tf.get_variable(shape=[c_in], name='cb', initializer=tf.initializers.zeros())

    weights[scope] = conv_weights


def init_convt_weight(weights, scope, k, c_in, c_out, spec_norm=True, zero=False, scale=1.0, classes=1):
    ...
    with tf.variable_scope(scope):
        ...
        conv_weights['b'] = tf.get_variable(shape=[c_in], name='b', initializer=tf.initializers.zeros())

        if classes != 1:
            conv_weights['g'] = tf.get_variable(shape=[classes, c_in], name='g', initializer=tf.initializers.ones())
            conv_weights['gb'] = tf.get_variable(shape=[classes, c_out], name='gb', initializer=tf.initializers.zeros())
        else:
            conv_weights['g'] = tf.get_variable(shape=[c_in], name='g', initializer=tf.initializers.ones())
            conv_weights['gb'] = tf.get_variable(shape=[c_out], name='gb', initializer=tf.initializers.zeros())
...
def init_res_weight(weights, scope, k, c_in, c_out, hidden_dim=None, spec_norm=True, res_scale=1.0, classes=1):
    ...
    init_conv_weight(weights, scope+'_res_c2', k, c_out, c_out, spec_norm=spec_norm, zero=True, scale=res_scale, classes=classes)

    if c_in != c_out:
        init_conv_weight(weights, scope+'_res_adaptive', k, c_in, c_out, spec_norm=spec_norm, scale=res_scale, classes=classes)

## Network forward helpers
def smart_conv_block(inp, weights, reuse, scope, use_stride=True, **kwargs):
    weights = weights[scope]
    return conv_block(inp, weights['c'], weights['b'], reuse, scope, scale=weights['g'], bias=weights['gb'], class_bias=weights['cb'], use_stride=use_stride, **kwargs)
...
def smart_convt_block(inp, weights, reuse, scope, output_dim, upsample=True, label=None):
    ...
    conv_output = tf.nn.conv2d_transpose(inp, cweight, [tf.shape(inp)[0], output_dim, output_dim, cweight.get_shape().as_list()[-2]], stride, 'SAME')
...
def smart_res_block(inp, weights, reuse, scope, downsample=True, adaptive=True, stop_batch=False, upsample=False, label=None, act=tf.nn.leaky_relu, dropout=False, train=False, **kwargs):
    gn1 = weights[scope+'_res_c1']
    gn2 = weights[scope+'_res_c2']
    c1 = smart_conv_block(inp, weights, reuse, scope+'_res_c1', use_stride=False, activation=None, extra_bias=True, label=label, **kwargs)

    if dropout:
        c1 = tf.layers.dropout(c1, rate=0.5, training=train)

    c1 = act(c1)
    c2 = smart_conv_block(c1, weights, reuse, scope+'_res_c2', use_stride=False, activation=None,  use_scale=True, extra_bias=True, label=label, **kwargs)
...
def conv_block(inp, cweight, bweight, reuse, scope, use_stride=True, activation=tf.nn.leaky_relu, pn=False,
               bn=False, gn=False, ln=False, scale=None, bias=None, class_bias=None, use_bias=False, downsample=False, stop_batch=False, use_scale=False, extra_bias=False, average=False, label=None):
...
def attention(inp, q, q_b, k, k_b, v, v_b, gamma, reuse, scope, stop_at_grad=False, seperate=False, scale=False, train=False, dropout=0.0):
    conv_q = conv_block(inp, q, q_b, reuse=reuse, scope=scope, use_stride=False, activation=None, use_bias=True, pn=False, bn=False, gn=False)
    conv_k = conv_block(inp, k, k_b, reuse=reuse, scope=scope, use_stride=False, activation=None, use_bias=True, pn=False, bn=False, gn=False)
